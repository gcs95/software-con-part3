...
beforeEach(function () {
    nuntachat = new StudentClass.Student('Nuntachat', 'Pattanasart', GenderClass.Gender.FEMALE, 'pploychat', 'correctpassword', '582115031');
});
...
test('getStudentID() works properly?', function(){
    expect(nuntachat.getStudentID()).toBe('582115031');
});

test('setStudentID() with correct works properly?', function(){
    expect(nuntachat.setStudentID('582115099')).toBe(true);
    expect(nuntachat.getStudentID()).toBe('582115099');
});

test('setStudentID() with incorrect works properly?', function(){
    expect(nuntachat.setStudentID('5821150996777')).toBe(false);
    expect(nuntachat.getStudentID()).toBe('582115031');
    expect(nuntachat.setStudentID('58211503q')).toBe(false);
    expect(nuntachat.getStudentID()).toBe('582115031');
});
