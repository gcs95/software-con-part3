const InstructorClass = require('./Instructor');
...
beforeEach(function () {
    nuntachat = new InstructorClass.Instructor('Nuntachat', 'Pattanasart', GenderClass.Gender.FEMALE, 'pploychat', 'correctpassword', '802354905');
});
...
test('getStudentID() works properly?', function(){
    expect(nuntachat.getInstructorID()).toBe('802354905');
});

test('setStudentID() with correct works properly?', function(){
    expect(nuntachat.setInstructorID('802354908')).toBe(true);
    expect(nuntachat.getInstructorID()).toBe('802354908');
});

test('setStudentID() with incorrect works properly?', function(){
    expect(nuntachat.setInstructorID('802354905p')).toBe(false);
    expect(nuntachat.getInstructorID()).toBe('802354905');
});
